...
import { getTrendDetail, type NotionBlock } from '@/lib/notion';
            ...
            <table className="w-full text-sm border-collapse">
              <tbody>
                {rows.map((row, ri) => {
                  ...
                  return (
                    <tr key={row.id} className={isHeader ? 'bg-emerald-700 text-white' : ri % 2 === 1 ? 'bg-gray-50' : 'bg-white'}>
                      {cells.map((cell, ci) => {
                        const Tag = isHeader ? 'th' : 'td';
                        return (
                          <Tag key={ci} className={`border-b border-r border-gray-200 px-4 py-2.5 text-left last:border-r-0 ${isHeader ? 'font-semibold border-emerald-600' : ''}`}>
...
export default async function TrendDetailPage({ params }: Props) {
  ...
  let detail;
  try {
    detail = await getTrendDetail(id);
  } catch {
    notFound();
  }
  ...
  return (
    <PageShell>
      <nav className="text-sm text-gray-400 mb-6">
        ...
        {' / '}
        <Link href={`/school-support/${school}/trend`} className="hover:text-gray-700">기출 경향</Link>
        {' / '}
      ...
      <div className="mb-8">
        ...
        {summary && (
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5">
            <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide mb-2">출제 경향 요약</p>
